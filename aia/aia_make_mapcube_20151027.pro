...
for i=0, ntim-1 do begin
    ...
    if ~exist(average) then begin
        ...
        if (oindex.exptime lt minexp) then flgs[i]=1
        ...
        if nind gt 0 then avgmap.data=avg(tmpmap[ind].data,2) else flgs[i]=1
    ...
    if exist(movie) then begin
        ...
        if flgs[i] eq 1 then xyouts,pos[0]+0.015,pos[1]+0.02,'flagged',color=cgcolor('white'),/norm
...
save,file=savfile,maps,reftim,flgs
