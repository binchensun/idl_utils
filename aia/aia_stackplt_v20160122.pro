pro aia_stackplt,mapcube,wave=wave,drawcut=drawcut,cutsav=cutsav,savfile=savfile,$
    cutwidth=cutwidth,cutang=cutang,dminmax=dminmax,movie=movie
...
if ~keyword_set(cutang) then cutang=5.*!dtor ;angular width of individual cut, in rad
...
if keyword_set(drawcut) then begin
    ...
    for i=0,5 do begin
        map=maps[mapinds[i]]
        plot_map,map,dmin=dminmax[0],dmax=dminmax[1],/log,position=pos[*,i]
    endfor
    timplt=''
    ...
    if strlen(timplt) le 1 then t_ind=mapinds[fix(timplt)] else $
        dt=min(anytim(maps.time)-anytim(timeplt),t_ind,/abs)
    mapplt=maps[t_ind]
    window,0,xs=800,ys=800
    plot_map,mapplt,dmin=dminmax[0],dmax=dminmax[1]
    ;draw the cut
    flag=0
    ans=''
    ...
    while flag eq 0 do begin
        ...
        while (!mouse.button eq 1) do begin
            ...
            !p.multi=0
            loadct,3
            plot_map,mapplt,dmin=dminmax[0],dmax=dminmax[1],/log
            ...
            !p.multi=[0,3,2]
            for i=0,5 do begin
                plot_map,maps[mapinds[i]],dmin=dminmax[0],dmax=dminmax[1],$
                    /log,position=pos[*,i]
                cgplots,xs_[0:n],ys_[0:n],psym=1,symsize=2.0,thick=2,/data,color='white'
...
if ~exist(flgs) then flgs=intarr(ntim)
good_ts=where(flgs eq 0, nt)
if nt le 0 then begin
    box_message,'No good time frames'
    return
endif
tims=tims[good_ts]
ncut=1
    ...
    for i=0, nt-1 do begin
        ...
        ;   print,'xedge and yedge', xedge, yedge
            ...
            if ni1 lt 1 or ni2 lt 1 then begin
                ...
                npix=n_elements(i2)
                imval0=fltarr(npix)
                for k=0, npix-1 do imval0[k]=mapdata[newxy[k,0],newxy[k,1]]
                intens[i,j,n]=mean(imval0)
                intens_sd[i,j,n]=stddev(imval0)
                npixs[i,j,n]=npix
            endelse    
        endfor
    endfor
endfor
if ncut eq 1 then intens=reform(intens,nt,ndist,1)
...
t_dist_rescale_aia, savfile, base_timran, bminmax, rminmax,tims,dists,$
    ...
    /b_log,ratio=[1,1],/smooth,smthsz=[1,3],dstep=1
spectro_plot,bytscl(bintensplt,min=bminmaxplt[0],max=bminmaxplt[1]),tims,dists,$
    ytit='Distance along cut (arcsec)',/xsty,/ysty
spectro_plot,bytscl(rintensplt,min=rminmaxplt[0],max=rminmaxplt[1]),tims,dists,$
    ytit='Distance along cut (arcsec)',/xsty,/ysty
!p.multi=0
