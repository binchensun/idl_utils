...
function diff_map_bc,map1,map2,rotate=rotate,ratio=ratio,missing=missing,nolimb=nolimb

if ~keyword_set(nolimb) then nolimb=1
if (not valid_map(map1)) or (not valid_map(map2)) then begin
 pr_syntax,'map=diff_map(map1,map2,[rotate=rotate,ratio=ratio])'
 return,-1
endif

if keyword_set(rotate) then dmap=drot_map(map2,ref=map1,nolimb=nolimb) else $
 dmap=map2
