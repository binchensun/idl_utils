...
;    be found at http://www.boulder.swri.edu/~buie/kbo/recov.  The files in this
...
pro tnorecov_loadobs, state, file
   ...
   if exists(file) then begin
      ...
      jdlast=jdparse(lastobs)
      daystolast=(*state).curjd-jdlast
      nf=n_elements(ra)
      (*state).nf = nf
      (*state).objfile = file
      ...
      (*state).ra     = ptr_new(ra)
      (*state).daystolast= ptr_new(daystolast)
...
pro tnorecov_plot, state
   ...
   if (*state).nf gt 0 then begin
      ...
      ; Time takes for each object to move minsep
      ...
      if count ne 0 then $
         timetoset[z] = (jdtrans[z]+horzha[z]/2.0d0/!dpi - curjd) * 24.0
      ...
      if count ne 0 then $
         timetoset[z] = 0.0
      z=where(timetoset ge 23.0 and timetoset lt 47.0,count)
      if count ne 0 then timetoset[z]=timetoset[z]-24.0
;if (*state).select ge 0 then begin
;print,strn((*state).select),' selected.'
;print,(*(*state).id)[(*state).select]
;print,timetoset[(*state).select]
;endif
      ...
      ; plot the ecliptic
      ecllon=dindgen(240) / 120.0 * 180.0 +0.01
...
;help,(*(*state).cand)
;z = where( (*(*state).toterr) lt (*state).minerr,count)
;print,strn(count),' with toterr < ',(*state).minerr
;z = where( (*(*state).toterr1) lt (*state).minerr1y,count)
;print,strn(count),' with toterr1 < ',(*state).minerr1y
;z = where( (*(*state).daystolast) lt (*state).minlast*365.25,count)
;print,strn(count),' with daystolast < ',(*state).minlast*365.25

   z = where( (*(*state).toterr) lt (*state).minerr or $
              (*(*state).toterr1) lt (*state).minerr1y or $
              (*(*state).toterr) gt (*state).maxerr or $
              (*(*state).daystolast) lt (*state).minlast*365.25 or $
              (*(*state).rmag)   lt (*state).brimag or $
              (*(*state).rmag)   gt (*state).fntmag, count )
...
pro tnorecov_eve, event
   ...
   case event_name of
      ...
      'BrightLimit': begin
         widget_control,(*state).brimagid,get_value=newlimit
         (*state).brimag = float(newlimit[0])
         widget_control, (*state).brimagid, $
            set_value=strn((*state).brimag,format='(1x,f10.2)')
         tnorecov_updatecand, state
         tnorecov_plot, state
      end

      'FaintLimit': begin
         widget_control,(*state).fntmagid,get_value=newlimit
         (*state).fntmag = float(newlimit[0])
         widget_control, (*state).fntmagid, $
            set_value=strn((*state).fntmag,format='(1x,f10.2)')
         tnorecov_updatecand, state
         tnorecov_plot, state
      end
      ...
      'MinErr1Y': begin
         widget_control,(*state).minerr1yid,get_value=newminerr
         (*state).minerr1y = float(newminerr[0])
         widget_control, (*state).minerr1yid, $
            set_value=strn((*state).minerr1y,format='(1x,f10.1)')
         tnorecov_updatecand, state
         tnorecov_plot, state
      end

      'MinLast': begin
         widget_control,(*state).minlastid,get_value=newlimit
         (*state).minlast = float(newlimit[0])
         widget_control, (*state).minlastid, $
            set_value=strn((*state).minlast,format='(1x,f10.2)')
         tnorecov_updatecand, state
         tnorecov_plot, state
      end
...
pro tnorecov,OBSFILE=obsfile,OBSCODE=obscode,AMCRIT=amcrit,DT=dt, $
   ...
   reffwhm = 1.2
   fwhm = 1.5
   maxexp = 600.0 ; not used (yet?)
   minlast = 0.0 ; minimum time, in years, since last observation
   brimag  =   0.0
   fntmag  =  23.5
   ...
   b2 = widget_base(b1,row=1)
   b3 = widget_base(b2,col=1)
   t1 = widget_label( b3, value='MinLast', /align_center, /dynamic_resize)
   minlastid = widget_text( b3, value=strn(minlast,format='(1x,f10.2)'),xsize=7, $
                           uvalue='MinLast',/align_center,/editable )
   b3 = widget_base(b2,col=1)
   t1 = widget_label( b3, value='Bright', /align_center, /dynamic_resize)
   brimagid = widget_text( b3, value=strn(brimag,format='(1x,f10.2)'),xsize=7, $
                           uvalue='BrightLimit',/align_center,/editable )
   b3 = widget_base(b2,col=1)
   t1 = widget_label( b3, value='Faint', /align_center, /dynamic_resize)
   fntmagid = widget_text( b3, value=strn(fntmag,format='(1x,f10.2)'),xsize=7, $
                           uvalue='FaintLimit',/align_center,/editable )
   b3 = widget_base(b2,col=1)
   t1 = widget_label( b3, value='Min1Ye', /align_center, /dynamic_resize)
   minerr1yid = widget_text( b3, value=strn(minerr1y,format='(1x,f10.2)'),xsize=7, $
                           uvalue='MinErr1Y',/align_center,/editable )
   ...
   state = ptr_new({ $
      ...
      brimag: brimag, $          ; Bright magnitude limit
      ...
      daystolast: ptr_new(), $   ; Days since last observation
      ...
      fntmag: fntmag, $          ; Faint magnitude limit
      ...
      minerr1y: minerr1y, $      ; Minimum error of interest at +1 year
      minlast: minlast, $        ; minimum number of years since last obs
      ...
      brimagid: brimagid, $      ; ID for bright mag limit
      ...
      fntmagid: fntmagid, $      ; ID for faint mag limit
      ...
      minerr1yid: minerr1yid, $  ; Minimum error of interest at +1 yr
      minlastid: minlastid, $    ; MinLast input ID
