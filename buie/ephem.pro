...
;-
pro ephem,in_jd,obs,code,object,ephemeris, DEBUG=debug, CACHE=cache, $
   ...
   if badpar(in_jd,5,[0,1],caller='EPHEM: (jd) ' ,npts=npts) then return
   ...
   if npts eq 1 and nobj gt 1 then begin
      jd=replicate(in_jd,nobj)
      npts=nobj
   endif else begin
      jd=in_jd
   endelse
