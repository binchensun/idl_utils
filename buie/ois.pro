...
; MWB changed 2011-12-27
;   z=where(mask eq 3 or mask eq 4, count)
   z=where(mask eq 3, count)
...
pro ois_fluxcheck,flux,minflux,maxflux,bad
   ...
   ;  but must get the first non-bad reference
   z=where(bad eq 0,count)
   if count eq 0 then return  ; this would be a problem
   fluxref=flux[z[0]]

   fmin = minflux*fluxref
   fmax = maxflux*fluxref
   ...
   while not done do begin
      if not silent then $
         print,'   --> post-kernel stamp filtering, pass ',strn(pass),', ', $
            strn(n_elements(xstamp)),' stamps left.'
      pass++
...
;               1 = pixel is too close to the edge of the image
;               2 = 
;               3 = pixel is flagged as "do not use"
;               4 = pixel is flagged as at or above saturation level
; OPTIONAL INPUT PARAMETERS:
...
pro ois_stamps,image,refer,in_fwhm,thresh, $
   ...
   robomean,fwhm,3.0,0.5,meanfwhm,bad=bad
   zb = where(bad eq 1,countzb)
   fwhmofftot = countzb - fwhmbadtot

   ; This next step will filter out any objects that are too close to the
   ;   edge that will fail the later basephote calculation
   ois_edgecheck,nx,ny,round(xcen),round(ycen),2*meanfwhm+1,bad
...
;print,'original sigkernel=',sigkernel,'  halfl=',halfl
;
;   ; second suggestion from Vallery Stanishev
;   sigk1 = meanfwhm/2.0*sqrt(alog(2.0)) ; convert to 1/e half-width
;   sigk2 = meanreffwhm/2.0*sqrt(alog(2.0)) ; convert to 1/e half-width
;   sigkernel = sqrt(sigk1^2+sigk2^2)
;   halfl=(fix(4.0*ceil(sigkernel)+2.0) > 3) < 26
;
;print,'suggested sigkernel=',sigkernel,'  halfl=',halfl
;print,'minmax of wstamp',minmax(wstamp)
;print,'alternate wstamp ',halfl+meanfwhm
...
;   if failed then return
   ...
   if not silent then $
      print,' ----> ','Stamp size range (dw): ',minmax(wstamp)
