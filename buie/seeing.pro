...
pro seeing,image,fwhm,objrad,nstars,ERROR=error, $
   ...
   objrad = fwhm0
;print,'start',fwhm0
   if radp then fwhmsize=objrad*1.5 else fwhmsize=objrad
   basphote,1.0,image,1.0,x,y,objrad,objrad+5,objrad+20,calcfwhmsize=fwhmsize, $
      /silent,/nolog,fwhm=fwhm1,xcen=newx1,ycen=newy1,radp=radp
   bad = bytarr(n_elements(x))
   ...
   z=where(bad eq 0,count)
;print,'1',objrad,fwhmsize,meanfwhm1
   ...
   if radp then fwhmsize=objrad*1.5 else fwhmsize=objrad
   basphote,1.0,image,1.0,x,y,objrad,objrad+5,objrad+20,calcfwhmsize=fwhmsize, $
      /silent,/nolog,fwhm=fwhm2,xcen=newx2,ycen=newy2,radp=radp
...
;print,'1',meanfwhm1
;print,'2',objrad,fwhmsize,meanfwhm2
