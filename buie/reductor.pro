...
pro reductor, DEBUG=debug, DATATABLE=datatable,DUMPOBJ=fulldump, $
              ...
              DVERBOSE=dverbose,SAVEALLPLOTS=saveallplots,LANDCAT=landcat
...
;  the last set of entries is for any other instrument.
   ...
   if badpar(transftable,[0,7],    0,  caller=self + '(TRANSFTABLE) ' )  $
                        then return
   if badpar(landcat,[0,7],0,CALLER=self+'(LANDCAT) ', $
         default=find_with_def('landphot.dat',!path)) then return
...
; ------------------------------------------------------------------------------
         ...
         'tr': begin
            ...
            ;  to the screen.
            stdcheck,landcat,pdata.stand[zfil], $
