...
;-
pro refgen,fnimage,astinfo,fnsrc,catfile,ref, $
       EXTENSION=extension,NOROTSCAN=norotscan,MAGLIM=maglim, $
       MAXERR=maxerr,SIGMAX=sigmax
   ...
   if badpar(maglim,[0,2,3,4,5],0,caller=self+'(MAGLIM) ', $
                                  default=30) then return
   if badpar(maxerr,[0,2,3,4,5],0,caller=self+'(MAXERR) ', $
                                  default=1) then return
   if badpar(sigmax,[0,2,3,4,5],0,caller=self+'(SIGMAX) ', $
                                  default=65000.0) then return
   if badpar(extension,[0,2,3],0,caller=self+'(EXTENSION) ', $
                                 default=0) then return
   if badpar(norotscan,[0,1,2,3],0,caller=self+'(NOROTSCAN) ', $
                                   default=0) then return
   ...
   peaksig = reform(image[fix(xpos+0.5),fix(ypos+0.5)],nlist)

   z=where(err lt maxerr and peaksig lt sigmax and mag lt 40.0,count)
   xpos=xpos[z]
   ypos=ypos[z]
   fwhm=fwhm[z]
   mag=mag[z]
   ...
   rotxc=0.
   rotyc=0.
   if xyerror gt 0 and not norotscan then begin
      frmdxyr,sxi,seta,cxi,ceta,-1.0,1.0,.1,xoff,yoff,angoff,xyerror, $
         tolerance=0.001,xout=newcxi,yout=newceta
