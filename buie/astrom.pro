...
pro astrom_getctr,thisexp,suff,cra,cdec,found_center,OUTDIR=outdir
...
;print,'gA [',thisexp,'] [',suff,']'
   ...
   if exists(outdir+'centers.dat') then begin

      ; Open the cache file for reading
      openr,lun,outdir+'centers.dat',/get_lun
      line=''
      done=0
...
;     (4)   RA from header
;     (5)   Dec from header
;     (6)   ra offset from header to center in radians (center-header)
...
pro astrom_svctr,thisexp,ra,dec,linfo,hdrinfo,outdir=outdir
   ...
   repwrite,outdir+'centers.dat',thisexp,cinfo
end
...
retry:
   ...
   cursor,xs2,ys2,3
   if !mouse.button eq 4 then return
   basphote,dv.gain,dv.image,dv.exptime,xs2,ys2,dv.objrad,dv.objrad+10, $
      dv.objrad+30,/nolog,/silent,xcen=xs2m,ycen=ys2m
...
pro astrom,root,fileno, $
   ...
   if badpar(magfilter,[0,2,3,4,5],0,CALLER='ASTROM: (MAGFILTER) ', $
                              default=1001.0) then return
   ...
   if badpar(in_path,[0,7],0,CALLER='ASTROM: (PATH) ', $
                             default='[[DEFAULT]]') then return
   if badpar(outdir,[0,7],0,CALLER='ASTROM: (OUTDIR) ', $
                             default='') then return
   ...
   if badpar(resfile,[0,7],0,CALLER='ASTROM: (RESFILE) ', $
                             default='[[DEFAULT]]') then return
   if badpar(in_starfile,[0,7],0,CALLER='ASTROM: (STARFILE) ', $
                              default='[[DEFAULT]]') then return
   ...
   if badpar(in_reffile,[0,7],0,CALLER='ASTROM: (REFFILE) ', $
                             default='[[DEFAULT]]') then return
   if badpar(refsave,[0,1,2,3],0,CALLER='ASTROM: (REFSAVE) ', $
                             default=0) then return
   ...
   if badpar(tweakobj,[0,1,2,3],0,CALLER='ASTROM: (TWEAKOBJ) ', $
                              default=0) then return
   if badpar(digits,[0,1,2,3],0,CALLER='ASTROM: (DIGITS) ', $
                              default=0) then return
   if badpar(forceterms,[0,7],[0,1],CALLER='ASTROM: (FORCETERMS) ', $
                              default='') then return

   ; no validation on forceval, let astsolve take care of it
...
; This section is only run once upon starting.
   ...
   if not exists(outdir+'Refstars') then file_mkdir,outdir+'Refstars'
   ...
   if fileno_type eq 0 then begin
      ...
      if digits eq 0 then begin
         ...
         fmt='(i'+strn(digits)+'.'+strn(digits)+')'
         suffix='.'+string(fileno,format=fmt)
      endelse
   endelse
...
;      raw=readfits(path+imfile,hdr,exten_no=extlist[ext],/silent,noscale=noscale)
      ...
      if in_starfile eq '[[DEFAULT]]' then begin
         in_starfile = objname[0] + exttag + twomass + '.cat'
      endif

      if strmid(in_starfile,0,1) eq '/' then starfile=in_starfile $
      else starfile=outdir+in_starfile
      ...
      doplast = exists(outdir+plastfile) and plastfile ne 'none'

      ; Read in astrometry information saved from last run.  If not found,
      ;   ask for information.
      if exists(outdir+astinf) then begin
         version=''
         openr,lun,outdir+astinf,/get_lun
         ...
         openw,lun,outdir+astinf,/get_lun
...
;print,'C goodsolution=',goodsolution
      ...
      ;   extraction.
      if not exists(starfile) or newcat or $
             (not doplast and plastfile ne 'none') then begin
         ...
         if objra lt 0.0 and center eq 0 then begin
            ...
            astrom_getctr,root+suffix+exttag,suffix+exttag,objra,objdec, $
                          found_center,outdir=outdir
...
;         psize=fix(sqrt(sz[1]^2+sz[2]^2)*pscale0*1.05+0.5+catpad)
      ...
      if not exists(starfile) or newcat then begin
         ...
         refnet,objra,objdec,psize,psize,maglim,maglim, $
            starfile,TWOMASS=twomass
         goodsolution = 0
      endif else begin
         killrefstars=0
      endelse
;print,'D goodsolution=',goodsolution
      ...
      if not doplast and plastfile ne 'none' then begin
         print,'       --> running plast....'
         plast,hdrinfo.jd,objra,objdec,psize,psize,outdir+plastfile, $
            title=imfile,obscode=obscode
         doplast = exists(outdir+plastfile)
      endif
      ...
      if doplast then begin
         rdplast,outdir+plastfile,pastname,astcode,pastmag,numobj
      ...
      for ie=0,n_elements(subexp)-1 do begin
         ...
         if subexp[ie] eq '[[DEFAULT]]' then begin
            if fileno_type eq 0 then begin
               ...
               endif else if fileno lt 10000 then begin
                  suff = string(fileno,format='(i4.4)')
               endif else begin
                  suff = string(fileno,format='(i5.5)')
               endelse
               suff = suff+exttag
            endelse
         endif else begin
         ...
         if in_reffile eq '[[DEFAULT]]' then begin
            reffile=outdir+addslash('Refstars')+suff+'.ref'
         endif else if strmid(in_reffile,0,1) eq '/' then begin
            reffile=in_reffile
         endif else begin
            reffile=outdir+addslash('Refstars')+in_reffile
         endelse
         if in_fnsrc eq '[[DEFAULT]]' then begin
            fnsrc = outdir+'Src/'+imfile+'.src'+exttag
         endif else if strmid(in_fnsrc,0,1) eq '/' then begin
            fnsrc = in_fnsrc
         endif else begin
            fnsrc = outdir+in_fnsrc
         endelse
         if fileno_type eq 0 then begin
            thisexp=root
         endif else begin
...
;            thisexp=root+'.'+suff
         ...
         ; Look for image center from previous reduction.
         astrom_getctr,thisexp,suff,cra,cdec,found_center,outdir=outdir
...
;print,'E goodsolution=',goodsolution
         ...
         ; display image
         if fullrefresh and not nodisplay then $
         ...
         ; xcref,ycref is almost always the center of the actual image.
...
;astrd2xy,linfo.raopt,linfo.decopt,linfo,xcopt,ycopt
;astrd2xy,linfo.raopt,linfo.decopt+0.00029,linfo,xcopt_n,ycopt_n
;astrd2xy,linfo.raopt+0.00029,linfo.decopt,linfo,xcopt_e,ycopt_e
;print,xcopt,ycopt
;print,'North is ',xcopt_n-xcopt,ycopt_n-ycopt,' from zero'
;print,'East  is ',xcopt_e-xcopt,ycopt_e-ycopt,' from zero'
         ...
         mantwostar=0
         if not exists(reffile) and exists(fnsrc) $
                                and ignoresrc eq 0 then begin
            ...
            if gap ne objrad then begin
               ...
               if not nostrictaper then begin
                  fiterror=1
                  return
               endif
            endif
            ...
            if magfilter lt 1000.0 then begin
               print,'median instrumental star mag',median(mag)
               print,'magcut is set to ',median(mag)+magfilter
               zz=where(mag lt median(mag)+magfilter,nlist0)
               print,strn(nlist0),' sources kept.'
               if nlist0 gt 0 then begin
                  xpos=xpos[zz]
                  ypos=ypos[zz]
                  fwhm=fwhm[zz]
                  mag =mag[zz]
...
;   for this to work.
            if not twostar then begin
               ...
               if doshells and ( maxrcrit lt 0 or (maxrcrit gt 0 and $
                                                  rcrit lt maxrcrit)) then begin
               ...
               frmdxdy,sx[zr2],sy[zr2],xpos[zr],ypos[zr],xoff,yoff,xyerror, $
                  nx=sz[1],ny=sz[2],scalefac=scalefac
               ...
               if xyerror gt 0 then begin
                  frmdxyr,sx[zr2],sy[zr2],xpos[zr],ypos[zr], $
                     ...
                     tolerance=rotscan[2],/silent,scalefac=scalefac
                  print,'     shfr> ',xoff,yoff,angoff,xyerror
               endif
               if xyerror eq 0 and maxoffset gt 0 and $
                  (abs(xoff) gt maxoffset or abs(yoff) gt maxoffset) then begin
                  print,'!!!ERROR offset exceeded maxoffset'
                  xyerror=10
               endif
            ...
            if twostar or xyerror eq 1 then begin
               ...
               if not twostar then $
                  logerror,outdir+errfile, $
                     'Bad auto correlation, file '+imfile+exttag
               ...
               if not valid then begin
                  logerror,outdir+errfile, $
                     'Manual solution aborted, file '+imfile+exttag
                  fiterror=1
                  return
               endif
...
restart_shell:
                  ...
                  if nstars lt n_elements(tterms) then begin
                     print,'Nstars=',strn(nstars),' and Nterms=',strn(nterms)
                     logerror,outdir+errfile, $
                        ['Fatal error!  pass 1 not enough stars lined up.'+ $
                           'Quitting on image', $
                         '   '+imfile+exttag]
                     countgood=0
                     fiterror=1
                  ...
                  astsolve,dx,dy,xi[zy],eta[zy],tterms,renormfac, $
                     ...
                     forceterms=forceterms,forceval=forceval, $
                     forcefilter=hdrinfo.filter
                  ...
                  if worstresid gt residcap then begin
                     if batch or ishell ne 0 then begin
                        ...
                        astrom_twostar,dv,linfo,sra,sdec,sx,sy,ssz,valid
                        if not valid then begin
                           logerror,outdir+errfile,'Manual solution aborted, file '+imfile+exttag
                           countgood=0
                           fiterror=1
                           goto,next_extension
                        endif else begin
            ...
            if countzq ne 0 then begin
               print,bel
               logerror,outdir+errfile,'Warning! poor star coverage on'+ $
                  strcompress(countzq)+' enneanants:  '+imfile+exttag
            endif

            if countzq gt maxbadenne then begin
               if batch then begin
                  logerror,outdir+errfile,'Batch processing on '+imfile+exttag+' aborted.'
                  countgood=0
                  fiterror=1
                  goto,next_extension
               endif
...
print,fnsrc
      ...
      retry:
            ...
            if count eq 0 then begin
               ...
               if strmid(ans,0,1) eq 'n' then begin
                  fiterror=1
                  return
               endif
...
;photographic refinement
         ...
         astsolve,dx,dy,xi,eta,terms,renormfac,bad,cxi,ceta, $
            ...
            forceterms=forceterms,forceval=forceval,forcefilter=hdrinfo.filter
         if worstresid gt residcap then begin
            logerror,outdir+errfile, $
...
;               oplot,astx,asty,psym=8,color=color1,symsize=0.2
         ...
         if countok gt 0 then begin
            ...
            print,' '
            magdifferr=stddev/sqrt(countok1-1)
            print,'Mean catalog magnitude difference ',magdiff, $
                  magdifferr,'from',strn(countok1),'stars', $
         ...
         fitdata[ext].photzperr = magdifferr

            ; Save the catalog star measurements to a binary file.
;         if exists(reffile) ne 1 or twostar then begin
         savereffile = not exists(reffile) or $
                       (refsave and (nstars ne countgood))
         if savereffile then begin
         ...
         if exists(outdir+'fitcoeff.dat') then begin
            ; promotes file type version of fitcoeff.dat
            astprmt,outdir+'fitcoeff.dat',renormfac=renormfac

            ; read the current file
            rdastfc,outdir+'fitcoeff.dat', $
                       t_ffn,t_ftype,t_xc,t_yc,t_prot,t_renormfac, $
                       t_cra, t_cdec,t_photzp,t_terms,t_coeffarr, $
                       t_ncoeffs,t_nlines,ERROR=error
            t_photzperr=t_photzp*0.0
            if error then begin
         ...
         if ze eq -1 and zx eq -1 then begin
            ...
            t_photzperr = [t_photzperr, magdifferr,magdifferr]
         ...
         ; save to file
         wrastfc,outdir+'fitcoeff.dat',t_ffn[zsort],t_ftype[zsort], $
         ...
         if photstars and countok gt 0 then begin
            ...
            hardim,bytscl(image,min=lowval,max=hival,top=255),0,255, $
               title=imfile+' '+objname[0],file=outdir+imfile+'.ps', $
            ...
            logfile=root+'.log'
            fnstars=root+'.stars'
            openw,lunstar,outdir+fnstars,/append,/get_lun
            openw,lunlist,outdir+imfile+'.lst',/get_lun
            ...
            free_lun,lunstar
            free_lun,lunlist
            if queue ne '' then hardcopy,outdir+imfile+'.lst',queue=queue
            if !version.os_family eq 'unix' then begin
               spawn,'sort '+outdir+fnstars+' | uniq > tmp.xxx; mv tmp.xxx '+outdir+fnstars
            endif
            filter=hdrinfo.filter
            if exists(outdir+logfile) then $
               logmanip,outdir+logfile,/JUSTCLEAN,DELETEFILE=imfile
            objnum=10
            basphote,gain,image,exptime,xpos[zok],ypos[zok],/silent, $
                     objrad,objrad+10,objrad+30,outdir+logfile,objnum,/ALTLOG, $
                     FNAME=imfile,FILTER=filter,JD=hdrinfo.jd,NAME=starname, $
                     PSCALE=linfo.pscale
; photographic refinement
            logmanip,outdir+logfile,/JUSTCLEAN
         ...
         if spot[0] gt -0.9e9 then begin
            ...
            for sn=0,nspots-1 do begin
               ...
               repwrite,outdir+'spot.dat',infostr,infostr+' '+ras+' '+decs
            endfor
         endif
         ...
         objid=0
         objtag=''
         if fileno_type ne 0 and exists(outdir+fnobj) and not skipobj then begin
            objprmt,outdir+fnobj,path=path,keylist=keylist
            rdoblist,outdir+fnobj,nobj,files,dt,offvals,xyvals,flags,nfiles
            ...
            if zf ne -1 then begin
               ...
               objtag=strmid(words[0],strlen(words[0])-2,2) + $
                      strmid(words[1],strlen(words[1])-3,3) + extstr
               for objid=0,nobj-1 do begin
                  ...
                  if flags[objid] eq 'y' then begin
                     ...
                     if x ge 0.0 and y ge 0.0 then begin
                        basphote,gain,image,exptime,x,y,objrad,objrad+10, $
                           objrad+30,/exact,/nolog,/silent,xcen=xm,ycen=ym, $
                           mag=mag0,fwhm=fwhm0
; photographic refinement
                        if not nodisplay then begin
                           oplot,[xm],[ym],psym=5,color=color4,symsize=1.5
                           xyouts,xm,ym,'  '+strtrim(string(objid),2), $
                              color=color4
                        endif
                        astmag=(mag0+magdiff) < 99.9
                        xi = asteval((xm-linfo.xcopt)/renormfac, $
                                     (ym-linfo.ycopt)/renormfac, $
                                     cxi,terms)/3600.0d0*!dpi/180.0d0
                        eta= asteval((xm-linfo.xcopt)/renormfac, $
                                     (ym-linfo.ycopt)/renormfac, $
                                     ceta,terms)/3600.0d0*!dpi/180.0d0
                        astsn2rd,xi,eta,linfo.raopt,linfo.decopt,ra,dec
                        rastr,ra,4,ras
                        ...
                        repwrite,outdir+resfile,thisexp,infostr
                        resfile='[[DEFAULT]]'
                        infostr=strcompress(string(xm,ym, $
                                                   format='(f10.3,1x,f10.3)'))
                        repwrite,outdir+'position.dat',tag,tag+infostr
                        if photstars then begin
                           basphote,gain,image,exptime,xm,ym,objrad,objrad+10, $
                                    objrad+30,logfile,objnum,/ALTLOG,/SILENT, $
                                    FNAME=imfile,FILTER=filter,JD=hdrinfo.jd, $
                                    NAME=tobjname,PSCALE=linfo.pscale,/EXACT
; photographic refinement
                        endif
                     endif else begin
                        repwrite,outdir+resfile,thisexp,''
                        repwrite,outdir+'position.dat',tag,''
                     endelse
                  endif else if flags[objid] eq '?' then begin
                     print,'WARNING: object line ',objid, $
                           ' has not yet been validated'
                     print,'             in file ',outdir+fnobj,', skipping.'
                     if exists(outdir+resfile) then begin
                        print,'deleting ',outdir+resfile
                        file_delete,outdir+resfile
                     endif
                  endif else begin
                     if exists(outdir+resfile) then begin
                        print,'deleting ',outdir+resfile
               ...
               print,'ERROR!  There is no valid file match in object file ', $
                     outdir+fnobj
            endelse

         endif else begin
            objres=file_search(outdir+objname[0]+'*.obj')
               ...
               if !mouse.button eq 1 then begin
                  ...
                  if photographic then begin
                     ...
                     photphot,image,xm,ym,dw,xcen,ycen,xfwhm,yfwhm,xflux, $
                        yflux,xback,yback,xcenerr,ycenerr,xfluxerr,yfluxerr, $
                        /debug
                     xm = xcen[0]
                     ym = ycen[0]
                  ...
                  if doneone then begin
                     ...
                     if photstars then begin
                        basphote,gain,image,exptime,xm,ym, $
                                 ...
                                 outdir+logfile,objnum,/ALTLOG,/SILENT, $
                                 FNAME=imfile, $
   ...
   ;==================================
      ...
      if pretty ne '[[DEFAULT]]' then begin
         ...
         hardim,bytscl(image,min=lowval,max=hival,top=255),0,255, $
            ...
            autosize=1,queue='chani',width=18.0,/noprint,file=outdir+pretty
...
;print,'F countgood=',countgood
         ...
         astcvt,'mn',0.,0.,fitdata[ext],'rd',newracen,newdeccen
         ; Save the final center
         astrom_svctr,thisexp,newracen,newdeccen,linfo,hdrinfo,outdir=outdir
...
;print,'A countgood=',countgood,'  toheader=',toheader
   ...
   if toheader and countgood gt 0 then begin
      ...
      if extlist[0] eq 0 then begin
         ...
         sxaddpar,hdr,'ASTPZPER',fitdata.photzperr, $
            ' uncertainty on photometric zero-point'
         ...
         for ext=0,numext-1 do begin
            ...
            sxaddpar,hdr,'ASTPHOZP',fitdata[ext].photzp, $
               ' Photometric zero-point from astrometric catalog'
            sxaddpar,hdr,'ASTPZPER',fitdata[ext].photzperr, $
               ' uncertainty on photometric zero-point'
            sxaddpar,hdr,'ASTAPER',objrad, $
               ' Photometric aperture for zero-point'
         endfor
