...
;                    boxmrad<<objrad).  It is not normal to set this but you
;                    might want to consider it in odd cases where you are
;                    using a photometry aperture smaller than the seeing.
;                    The FWHM is often underestimated if the size of the
;                    region for the calculation is too small.
...
;    RADP    - Flag, if set uses a radial profile gaussian fit to determine
;                  the FWHM.  This is more accurate if the seeing is gaussian
;                  and well behaved.  This option will be slower and slightly
;                  more prone to failure, particularly for faint sources.
...
pro basphote, gain, image, exptime, xloc, yloc, radius, sky1, sky2, $
              ...
              NOMEXT=in_nomext, ONCHIP=out_onchip,RADP=radp, $
              ...
              ZPOINT=in_zpoint,CALCFWHMSIZE=calcfwhmsize ;, TIMEIT=timeit
   ...
   if badpar(calcfwhmsize,[0,2,3,4,5],0,CALLER=self+'(CALCFWHMSIZE) ', $
                DEF=radius[n_elements(radius)-1]) then return
   ...
   if badpar(radp,[0,1,2,3],0,CALLER=self+'(RADP) ',DEF=0) then return
   ...
   if keyword_set(altlog) then begin
      ...
      if badpar(in_pscale,[4,5],0,CALLER=self+'(pscale) ') then error=1
   ...
   ;      if timeit then cputime,time2
         if boxmrad gt 0.0 then begin
            msz=fix(boxmrad+0.5)
            boxm,wimage,objxloc,objyloc,msz,msz,xmax,ymax
         endif else begin
            xmax=objxloc
            ymax=objyloc
         endelse
   ...
   ;      if timeit then cputime,time8
         ...
         if pscale gt 0.0 then begin
            if radp then begin
               radp,sub-tmp_skymean,xcen1-lx,ycen1-ly,r,im,tmp_fwhm,/conly
            endif else begin
               xsum=fltarr(hx-lx+1)
               ysum=fltarr(hy-ly+1)
         ...
         if fwhm[i,frame] gt 99.994 or fwhm[i,frame] lt -9.98 then $
            fwhm[i,frame] = -1.0
