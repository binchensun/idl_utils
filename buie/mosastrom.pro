...
pro mosastrom,name,xoffset,yoffset,rotang, $
       ...
       NEWRENORM=newrenorm,IMACS=imacs,RDIR=rdir
   ...
   if badpar(path,[0,7],0,caller=self+'(PATH) ', $
                                default='') then return
   if badpar(rdir,[0,7],0,caller=self+'(rdir) ', $
                                default='') then return
   if rdir ne '' then rdir=addslash(rdir)
   if badpar(srcdir,[0,7],0,caller=self+'(SRCDIR) ', $
                                default=rdir+'Src/') then return
   if badpar(refdir,[0,7],0,caller=self+'(REFDIR) ', $
                                default=rdir+'Refstars/') then return
   if badpar(subaru,[0,1,2,3],0,caller=self+'(SUBARU) ', $
                                default=0) then return
   if badpar(imacs,[0,1,2,3],0,caller=self+'(IMACS) ', $
                                default=0) then return
   ...
   if subaru then begin
      ...
      fnref  = name+sidx+'.ref'
      chipno = idx
   endif else if imacs then begin
      nexten=8
      idx=indgen(nexten)
      sidx=strcompress(string(idx+1),/remove_all)
      imname = name+'x'+sidx+'.fits'
      exten  = replicate(0,8)
      fitname = name+'x'+sidx
      fnsrc  = fitname+'.src'
      fnref  = fitname+'.ref'
   ...
   if subaru then begin
      ...
      starfile = strupcase(hdrinfo.object)+'_'+strb36(chipno)+'.cat'
   endif else begin
   ...
   rdastfc,rdir+'fitcoeff.dat',ffn,ftype,xc,yc,prot,renormfac,cra,cdec,photzp, $
   ...
   for i=0,nimages-1 do begin
      ...
      print,'running refgen for chip ',exten[idx[i]]
