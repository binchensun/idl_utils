...
;-
pro siftstar,fn,REDDIR=reddir,KEYLIST=keylist,VERBOSE=verbose
   ...
   ;   and use.
   if verbose then print,'Read source list ',fnsrd
   ...
   if verbose then begin
      print,strn(n_elements(fwhm)),' sources in source list.'
      print,'Mean FWHM from source list is ',avgfwhm
   endif

   cdir=ddir+addslash(root)+'cal/'
   if not exists(cdir+fn) then begin
      print,self,'Error: image ',cdir+fn,' does not exist.'
      return
   endif
   ...
   ;   aperture.
   if verbose then print,'Large aperture photiso call'
   photiso,gain,image,hdrinfo.exptime,xloc,yloc,radius,sky1,sky2,ngood,zgood, $
      PSCALE=pscale,RDNOISE=rdnoise, $
      ERR=err,FLERR=flerr,FLUX=flux,FWHM=fwhm1,MAG=mag,XCEN=xcen,YCEN=ycen, $
      SKYMEAN=skymean,SKYERR=skyerr,VERBOSE=verbose
   if verbose then print,strn(ngood),' separated sources kept.'
   ...
   if verbose then print,'Write ',sdir+fnsr1
   ...
   if verbose then print,'Small aperture photiso call'
   photiso,gain,image,hdrinfo.exptime,xloc,yloc,avgfwhm,skymean,-1.0*skyerr, $
      ngood2,zgood2, $
      PSCALE=pscale,RDNOISE=rdnoise,VERBOSE=verbose, $
      ERR=err,FLERR=flerr,FLUX=flux,FWHM=fwhm2,MAG=mag,XCEN=xcen,YCEN=ycen
   if verbose then print,strn(ngood2),' isolated+tight sources kept.'
   ...
   if verbose then print,strn(n_elements(good)),' tight sources kept.'

   sxaddpar,hdr1,'OBJRAD',avgfwhm

   if verbose then print,'Write ',sdir+fnsr2
