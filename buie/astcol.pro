...
pro astcol,OBSCODE=obscode,SAVEDIR=savedir,NONEWCODE=nonewcode,NOXREF=noxref, $
   NODB=nodb,DES=des,REPOST=repost,ALLTNO=alltno
   ...
   if badpar(alltno,[0,1,2,3],0,CALLER=self+'(ALLTNO) ',default=0) then return
   ...
   if not nodb or des or alltno then openmysql,dblun,'des'
   ...
   if des then begin
      ...
      mysqlquery,dblun,cmd,desobj,format='a',ngood=numdesobj
   endif else begin
      desobj=''
      numdesobj=0
   endelse

   ; if ALLTNO flat set, we need to get the list of known TNOs from dclass
   if alltno then begin
      cmd='select objectid from dclass;'
      mysqlquery,dblun,cmd,tnolist,format='a',ngood=numtnos
   endif else begin
      tnolist=''
      numtnos=0
   endelse
   ...
   for i=0L,nfiles-1 do begin
      ...
      if numtnos gt 0 then begin
         z = where(strupcase(obj) eq tnolist,count)
         if count ne 0 then begin
            if numdesobj eq 0 then begin
               desobj=strupcase(obj)
               numdesobj=1
            endif else begin
               desobj=[desobj,strupcase(obj)]
               numdesobj++
            endelse
         endif
      endif
   ...
   free_lun,lunast
   if not nodb or des or alltno then free_lun,dblun
   ...
   if numdesobj gt 0 then begin
      desobj=desobj[uniq(desobj,sort(desobj))]
      outobj  = savedir+date+'.kobj'
      openw,lun,outobj
      for i=0,n_elements(desobj)-1 do begin
         if desobj[i] ne '' then printf,lun,desobj[i]
      endfor
      free_lun,lun
   endif
