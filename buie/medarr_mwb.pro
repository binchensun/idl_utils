...
;-
pro medarr_mwb,inarr,outarr,BAD=bad,NTOPCLIP=ntopclip
   ...
   if badpar(ntopclip,[0,1,2,3],0,caller=self+'(NTOPCLIP) ', $
                default=0) then return

   if nbad ne 1 and nbad ne nin then begin
   ...
   for row = 0,nrow-1 do begin
      for col = 0,ncol-1 do begin
         if usebad then begin
            planes = where( bad[col,row,*] eq 0, narr)
         endif else begin
            narr=dimen[2]
            planes=indgen(narr)
         endelse
         if narr gt 0 then begin
            ...
            ord = sort(allpix)
            if ntopclip gt 0 then begin
               if ntopclip ge narr then begin
                  outarr[col,row] = 0
                  continue
               endif
               ord=ord[0:(-1-ntopclip)]
               narr = narr-ntopclip
            endif
            even = fix(narr/2)*2 eq narr
            if even then begin 
