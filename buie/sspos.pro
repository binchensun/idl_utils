...
pro sspos,TIME=time,AMCRIT=amcrit,OBSFILE=obsfile,OBSCODE=obscode,DEBUG=debug, $
          ...
          STWILIGHT=stwilight,APO=apo,DT=dt
   ...
   if badpar(dt,[0,1,2,3,4,5],0,caller='SSPOS: (DT) ',default=-1) then return

   dt = dt<60 ; don't let this be too long.
   ...
   while obid ne 'end' do begin
      if dt le 0 or obid eq '' then begin
         in_obid=''
      ...
      if goodobj then begin
         ...
         decrate = (eph[1,2] - eph[1,1])*180.0d0/!dpi*3600.0/ $
                     ((nexthr-prevhr)*24.0d0)
         rarate  = (eph[0,2] - eph[0,1])*180.0d0/!dpi*3600.0*cos(eph[1,2])/ $
                     ((nexthr-prevhr)*24.0d0)
         ...
         if apo then begin
            ...
            if earth[0] gt 0.002 then begin
               if rarate gt 3600.0 then begin
                  print,'   Earth to Object  ',earth[0],' AU ', $
                        '   RA rate      ',rarate/3600.0,  ' deg/hr', $
                        format='(a,f7.3,a,a,f9.3,a)'
               endif else if rarate gt 60.0 then begin
                  print,'   Earth to Object  ',earth[0],' AU ', $
                        '   RA rate      ',rarate/60.0,  " '/hr", $
                        format='(a,f7.3,a,a,f9.3,a)'
               endif else begin
                  print,'   Earth to Object  ',earth[0],' AU ', $
                        '   RA rate      ',rarate,  ' "/hr', $
               ...
               if rarate gt 3600.0 then begin
                  print,'   Earth to Object  ',round(earth[0]*1.495979e8),' km ', $
                        '   RA rate      ',rarate/3600.0,  ' deg/hr', $
                        format='(a,i7,a,a,f9.3,a)'
               endif else if rarate gt 60.0 then begin
                  print,'   Earth to Object  ',round(earth[0]*1.495979e8),' km ', $
                        '   RA rate      ',rarate/60.0,  " '/hr", $
                        format='(a,i7,a,a,f9.3,a)'
               ...
               endelse
            endelse
            if decrate gt 3600.0 then begin
               print,'   Phase angle      ',phang[0],'  deg', $
                     '   Dec rate     ',decrate/3600.0, ' deg/hr', $
                     format='(a,f6.2,a,a,f9.3,a)'
            endif else if decrate gt 60.0 then begin
               print,'   Phase angle      ',phang[0],'  deg', $
                     '   Dec rate     ',decrate/60.0, " '/hr", $
                     format='(a,f6.2,a,a,f9.3,a)'
            endif else begin
               print,'   Phase angle      ',phang[0],'  deg', $
                     '   Dec rate     ',decrate, ' "/hr', $
         ...
         if dt gt 0 then obid=''
         goto,skip
      endelse
      if dt gt 0 then wait,dt
skip:
   endwhile
