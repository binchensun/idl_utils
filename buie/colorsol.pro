...
pro colorsol, stand,fil,jd,am,serial,inst,instsig, $
   ...
   if badpar(trans1,[4,5],[1,2],caller=self+'(trans1) ', $
                                npts=n9,rank=trank1) then return
   if badpar(trsig1,[4,5],[1,2],caller=self+'(transig1) ', $
                                npts=n10,rank=trank2) then return
   if badpar(jdref1,5,  [0,1],caller=self+'(jdref1) ',npts=n9a) then return
   if badpar(trans2,[4,5],[1,2],caller=self+'(trans2) ', $
                                npts=n11,rank=trank3) then return
   if badpar(trsig2,[4,5],[1,2],caller=self+'(transig2) ', $
   ...
   if trank1 eq 1 then begin
      ...
      sz=size(trans1,/dimen)
      if sz[0] ne 5 then begin
         print,self+'Error!  trans1 array must be 5xN'
         return
      endif
      ntran=sz[1]
      sz=size(trsig1,/dimen)
      if sz[0] ne 5 then begin
         print,self+'Error!  transig1 array must be 5xN'
         return
      endif
      if n9a ne ntran then begin
         help,trans1,jdref1
      ...
      sz=size(trans2,/dimen)
      if sz[0] ne 5 then begin
         print,self+'Error!  trans2 array must be 5xN'
         return
      endif
      ntran=sz[1]
      sz=size(trsig2,/dimen)
      if sz[0] ne 5 then begin
         print,self+'Error!  transig2 array must be 5xN'
         return
      endif
      if n11a ne ntran then begin
         help,trans2,jdref2
   ...
   color = replicate(refcolor,n_elements(inst))
   colorsig = replicate(refcolor,n_elements(inst))
   blanks='                   '
   countchange = 0
...
again:
   ...
   repeat begin
      ...
      inst2std,jd[z1],am[z1],inst[z1],instsig[z1],color[z1],colorsig[z1], $
         ...
         allmag1,allerr1,tidx=lindgen(count1),refcolor=refcolor
      ...
      inst2std,jd[z2],am[z2],inst[z2],instsig[z2],color[z2],colorsig[z2], $
         ...
         allmag2,allerr2,tidx=lindgen(count2),refcolor=refcolor
   ...
   if edit then begin
      ...
      if count1 ne 0 and count2 ne 0 then begin
         ...
         inst2std,jd[z1e],am[z1e],inst[z1e],instsig[z1e], $
            ...
            allmag1e,allerr1e,tidx=lindgen(count1),refcolor=refcolor
         ...
         inst2std,jd[z2e],am[z2e],inst[z2e],instsig[z2e], $
            ...
            allmag2e,allerr2e,tidx=lindgen(count2),refcolor=refcolor
