...
pro futureob,start,length,obs,object,FULLNAME=fullname,DT=dt
   ...
   ra=trimrank(eph[0,*]) * !radeg / 15.0
   dec=trimrank(eph[1,*]) *!radeg
   ssgeom,eph,r,d,p,e
   jd2year,jd,yr
   ...
   setwin,2
   plot,yr,ra,chars=cs,ytitle='RA'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],ra[i1[i]:i2[i]],color=color,thick=th
   plot,yr,dec,chars=cs,ytitle='Dec'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],dec[i1[i]:i2[i]],color=color,thick=th
   plot,ra,dec,chars=cs,xtitle='RA',ytitle='Dec'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,ra[i1[i]:i2[i]],dec[i1[i]:i2[i]],color=color,thick=th
