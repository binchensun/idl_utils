...
;-
pro parsekey,header,keylist,info,UNKNOWN=unknown,VERBOSE=verbose

   self='PARSEKEY: '
   if badpar(header, 7,1,caller=self+'(header) ') then return
   if badpar(keylist,7,2,caller=self+'(keylist) ') then return
   if badpar(verbose,[0,1,2,3],0,caller=self+'(VERBOSE) ', $
                                 default=0) then return
   if badpar(unknown,[0,7],1,caller=self+'(UNKNOWN) ', $
                             default=['',''],npts=nvalu) then return

   if nvalu ne 2 then begin
      print,self,' keyword UNKNOWN must have exactly two elements.'
      return
   endif
         ...
         'FILTER' : BEGIN
            CASE keylist[1,j] OF
               'K' : begin
                  ...
                  z=where(answer eq unknown[0],count)
                  if count ne 0 then answer[z]=unknown[1]
   ...
   tloc = strpos(info.ut,'T')
   if tloc ge 0 then info.ut = strmid(info.ut,tloc+1)
   x = BYTE( info.ut )
