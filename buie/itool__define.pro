...
pro itool::display
   ...
   widget_control, (*pstate).animatesaveid,SENSITIVE=((*pim_parms).nframes gt 1)
...
function itool::init, oimage,$
   ...
   if scrsize[0] gt 1300 then begin
      ...
      if scrsize[1] ge 1024 then begin
         xvisdef = 900
         yvisdef = 640
      endif else begin
         xvisdef = 900
   ...
   state = {$
      ...
      animatesaveid: 0L, $          ; Animation save button id.
   ...
   state.animatesaveid = widget_button(animatemenuid, VALUE='Save',$
      EVENT_PRO='itool_animatesave_event')
...
; ----------------------------------------------------------------------------
; Procedure itool_animatesave_event
; Event handler for the Animate save button.
; ----------------------------------------------------------------------------
pro itool_animatesave_event, event
   compile_opt hidden
   ...
   ovid=idlffvideowrite('animation.mp4')
   vidstream=ovid.addvideostream(xs,ys,fps)
;print,ovid.getcodecs(/video)
   ...
   for j=0, nframes-1 do begin
      ...
      bim=bytscl(rebin((*pimage)[*,*,j], xs, ys, /SAMPLE), $
            min=(*pim_parms).curmin[j], max=(*pim_parms).curmax[j], $
            TOP=(*pstate).ncolors-1)
      cim=bytarr(3,xs,ys)
