...
pro moscal,root,outdir,start,nframes,bias,dark,flat, $
   ...
   OVERSCAN=in_overscan,CROP=in_crop,GAINCOR=in_gaincor,FLOAT=fltdata
   ...
   if badpar(flat,   [0,7],0,caller=self+'(flat) ', $
                                    default='[[none]]') then return
   if badpar(fltdata,[0,1,2,3],0,caller=self+'(FLOAT) ',default=0) then return
