;+
Pro fieldlines_fff_white, bx_in, by_in, bz_in, ix_in, iy_in, iz_in, x_in, y_in, z_in, $
                    angx = angx, angz = angz, $
...
;
  ...
  miss = 255                      ;missing is white
  c_color = [150, 200, 250]
;
  miss = 255                      ;Get polynomial coeff for warp 
...
; outline box by drawing edges (dashed)
    plots, [xmax, xmax], [ymin, ymax], [zmin, zmin], /t3d, /data, $
      linestyle = 2, color = black, thick = thick_x
...
; h is stepsize, by default 1/8 of minimum gridsize
  ...
  xyouts, 0.07, 0.89, ttl, charsize = 1.3
; draw axes
  thick = thick_x
  plots, [xmin, xmin], [ymin, ymax], [zmin, zmin], /t3d, /data, $
    color = black, thick = thick
...
;
  charthick = 1.75
  xyouts, xmax+.05, ymin, z = zmin, 'X', /t3d, /data, size = 2, $
    color = black, charthick=charthick
  xyouts, xmin, ymax+.05, z = zmin, 'Y', /t3d, /data, size = 2, $
    color = black, charthick=charthick
  xyouts, xmin, ymin, z = zmax/2+0.05, 'Z', /t3d, /data, size = 2, $
    color = black, charthick=charthick
