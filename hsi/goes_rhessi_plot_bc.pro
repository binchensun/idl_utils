...
PRO goes_rhessi_plot_bc, time=time, $
  STACKED=stacked, $
  orbit=orbit, $
  black_bk=black_bk, $
  RHESSI=rhessi, $
...
FOR i = 0, nplots-1 DO BEGIN
  ...
  ;if starting time is later than the current time, skip goes plot
  get_utc,time_now,/vms
  if tplot[0] lt anytim(time_now) then begin
  ...
  ;if the starting time is later than the current time, plot the no-good times only
  ; just plot axes and labels first
      yrange[0] = 1.d-9
      yrange[1] = 1.d-2
      title = 'RHESSI nights+SAA starting from ' + anytim(tplot[0],/vms)
      utplot, anytim(/ext,tplot), yrange, /nodata, yrange=yrange, timerange=tplot, title=title, $
        ytitle=ytitle, /ylog, ystyle=ystyle, yminor=10, ymargin=[3.5,2.], $
        /xstyle, xmargin=[8,3], xtitle='', $
        position=position, $
