...
;-
;
pro ssc_plot_where_bc, date, planet=planet,inner=inner, outer=outer, ulysses=kulysses, $
                    ...
                    position=position, charsize=charsize, symsize=symsize, _extra=_extra
on_error, 2
;
...
circle_sym, /fill
;  Setup charsize
if ~keyword_set(charsize) then charsize=1.5 
;
;  Setup symsize
...
if keyword_set(planet) then oplot, orbit[1,*], orbit[0,*], line=2, color=grey
xx = mercury[1]
yy = mercury[0]
if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and (yy le !y.crange[0]) and $
  (yy ge !y.crange[1]) then begin
    plots, xx, yy, psym=8, color=grey
    if not keyword_set(outer) then xyouts, xx, yy, ' Mercury', color=grey, charsize=charsize
endif
;
...
if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and (yy le !y.crange[0]) and $
  (yy ge !y.crange[1]) then begin
    plots, xx, yy, psym=8, color=grey
    if not keyword_set(outer) then xyouts, xx, yy, ' Venus', color=grey, charsize=charsize
endif
;
...
if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and (yy le !y.crange[0]) and $
  (yy ge !y.crange[1]) then begin
    plots, xx, yy, psym=8, color=grey
    if not keyword_set(outer) then xyouts, xx, yy, ' Mars', color=grey, charsize=charsize
endif
;
if keyword_set(outer) then begin
    ...
    xx = jupiter[1]
    yy = jupiter[0]
    if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=grey
        xyouts, xx, yy, ' Jupiter', color=grey, charsize=charsize
    endif
;
    ...
    if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=grey
        xyouts, xx, yy, ' Saturn', color=grey, charsize=charsize
    endif
;
    ...
    if keyword_set(planet) and (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=grey
        xyouts, xx, yy, ' Uranus', color=grey, charsize=charsize
    endif
endif
;
...
if keyword_set(kulysses) then begin
    ...
    if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        ...
        xyouts, xx, yy, ' Ulysses', color=purple, charsize=charsize
    endif
endif
;
if keyword_set(kmessenger) then begin
    ...
    if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        ...
        xyouts, xx, yy, ' Messenger', color=orange,charsize=charsize
    endif
endif
;
...
if keyword_set(krosetta) then begin
    ...
    if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=purple
        xyouts, xx, yy, ' Rosetta', color=purple, charsize=charsize
    endif
endif
;
...
if keyword_set(kmaven) then begin
    ...
    if (tai ge tai0) and (tai le tai1) then begin
        ...
        if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
          (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
            plots, xx, yy, psym=8, color=orange
            xyouts, xx, yy, ' MAVEN', color=orange, charsize=charsize
        endif
    endif
endif
;
...
if keyword_set(kvesta) then begin
    ...
    if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=purple
        xyouts, xx, yy, ' Vesta', color=purple, charsize=charsize
    endif
endif
;
if keyword_set(kceres) then begin
    ...
    if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and $
      (yy le !y.crange[0]) and (yy ge !y.crange[1]) then begin
        plots, xx, yy, psym=8, color=orange
        xyouts, xx, yy, ' Ceres', color=orange, charsize=charsize
    endif
endif
;
...
plots, symsize=2*symsize, 0, earth[0], psym=8, color=green
if not keyword_set(outer) then xyouts, 0, earth[0], '  Earth', color=green, charsize=charsize
;
;  Overplot the Sun.
;
plots, symsize=3*symsize, 0, 0, psym=8, color=yellow
if not keyword_set(outer) then xyouts, 0, 0, '  Sun', color=yellow, charsize=charsize
;
;  Overplot the positions of the two STEREO spacecraft.  Draw lines from the
;  Sun through each spacecraft.
;
if (sta[1] ge !x.crange[0]) and (sta[1] le !x.crange[1]) and $
  (sta[0] le !y.crange[0]) and (sta[0] ge !y.crange[1]) then begin
    oplot, [0,20*sta[1]], [0,20*sta[0]], line=1
    plots, symsize=symsize, sta[1], sta[0], psym=8, color=red
    if not keyword_set(outer) then xyouts, sta[1], sta[0], ' A', $
      color=red, charsize=charsize
endif
if (stb[1] ge !x.crange[0]) and (stb[1] le !x.crange[1]) and $
  (stb[0] le !y.crange[0]) and (stb[0] ge !y.crange[1]) then begin
    oplot, [0,20*stb[1]], [0,20*stb[0]], line=1
    plots, symsize=symsize, stb[1], stb[0], psym=8, color=blue
    if not keyword_set(outer) then xyouts, stb[1], stb[0], ' B', $
      color=blue, charsize=charsize
endif
;
;  Reset the scale.
;
...
;
    ...
    plots, symsize=symsize, -moon[1], -moon[0]+earth[0], psym=8, color=grey
    if !x.crange[1] lt 0.03 then $
      xyouts, -moon[1], -moon[0]+earth[0], ' Moon', color=grey, charsize=charsize
;
;  Overplot, the position of Earth.
;
    plots, symsize=3*symsize, 0, 0, psym=8, color=green
    xyouts, 0, 0, '   Earth', color=green, charsize=charsize
;
...
;
    if (-sta[1] ge !x.crange[0]) and (-sta[1] le !x.crange[1]) and $
      (-sta[0]+earth[0] le !y.crange[0]) and (-sta[0]+earth[0] ge !y.crange[1]) then begin
        plots, symsize=symsize, -sta[1], -sta[0]+earth[0], psym=8, color=red
        xyouts, -sta[1], -sta[0]+earth[0], ' A', color=red, charsize=charsize
    endif
    if (-stb[1] ge !x.crange[0]) and (-stb[1] le !x.crange[1]) and $
      (-stb[0]+earth[0] le !y.crange[0]) and (-stb[0]+earth[0] ge !y.crange[1]) then begin
        plots, symsize=symsize, -stb[1], -stb[0]+earth[0], psym=8, color=blue
        xyouts, -stb[1], -stb[0]+earth[0], ' B', color=blue, charsize=charsize
    endif
;
